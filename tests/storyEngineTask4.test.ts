...
describe('Story Engine Task 4 regressions', () => {
  ...
  it('30 semantic validator uses QUALITY', () => { expect(getStoryModelRoute('STORY_VALIDATOR_SEMANTIC')).toMatchObject({ tier: 'QUALITY', requiredInStrictMode: true, allowFastFallback: false }); expect(getStoryModelCandidates('STORY_VALIDATOR_SEMANTIC')).toEqual(['gemini-3.1-pro-preview']); });
