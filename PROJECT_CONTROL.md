...
- New-build real AI Studio acceptance is partially complete: GitHub `main` is synchronized, the app opens without the custom access-code screen, `APP_DEPLOYMENT_MODE=private-aistudio` is configured, the default server-only Gemini profile is recognized by masked fingerprint, and a real `gemini-3.5-flash` health call passed in 1145 ms. A `gemini-3.1-pro-preview` test returned a normalized temporary rate-limit message rather than false daily depletion. Multi-profile failover, streaming, and official Story Chapters 1-2 remain pending until an additional owner Gemini Secret is registered. The older live-runtime evidence below is historical and must not be treated as acceptance of the new build.
