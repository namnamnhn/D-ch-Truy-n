...
> Current multi-profile release status (2026-08-30): local code, regression, build, credential-boundary, PDF, and production-server gates pass (26 files / 389 tests). Real AI Studio now passes direct entry, private runtime activation, masked discovery of the default profile plus `GEMINI_PROFILE_1_API_KEY`, and an ordinary `gemini-3.5-flash` call; `gemini-3.1-pro-preview` rate limiting is reported as temporary rather than falsely called daily exhaustion. Multi-profile execution-target failover, streaming, and Story Chapters 1–2 acceptance remain pending; they are not claimed by the historical evidence below.
